...
def negSamplingLossAndGradient(
    ...
    tmp = sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec))
    loss -= np.log(tmp)

    gradOutsideVecs[outsideWordIdx] += centerWordVec * (tmp - 1.0)
    gradCenterVec += outsideVectors[outsideWordIdx] * (tmp - 1.0)

    
    U = outsideVectors[negSampleWordIndices]
    tmp = sigmoid(-np.dot(U,centerWordVec))
    loss -= np.sum(np.log(tmp))
    gradCenterVec -= np.dot((tmp-1),U)
    for i, neg_index in enumerate(negSampleWordIndices):
        gradOutsideVecs[neg_index] += (1 - tmp[i]) * centerWordVec # I need a for loop to take into account multiple negative appearences
